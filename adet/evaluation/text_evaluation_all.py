...
class TextEvaluator():
    ...
    def __init__(self, dataset_name, cfg, distributed, output_dir=None):
        ...
        elif self.voc_size == 148:  # A 32 a 60
            self.CTLABELS = [' ', '!', '"', '#', '$', '%', '&', "'", '(', ')', '+', ',', '-', '.', '/', '0', '1', '2', '3', '4', '5',
        ...
        elif "rumsey" in dataset_name:
            self._text_eval_gt_path = "datasets/evaluation/gt_rumsey.zip"
            self._word_spotting = True
            self.dataset_name = "rumsey"
        elif "ign" in dataset_name:
            self._text_eval_gt_path = "datasets/evaluation/gt_ign.zip"
            self._word_spotting = True
            self.dataset_name = "ign"
    ...
    def to_eval_format(self, file_path, temp_dir="temp_det_results"):
        ...
        with open(file_path, 'r') as f:
            ...
            if "rumsey" in self.dataset_name:
                json_file = PathManager.get_local_path(self._metadata.json_file)
                with open(json_file, 'r') as f:
                    dataset = json.load(f)
                # Create a dictionary mapping from the dataset
                for image in dataset['images']:
                    rumsey_id2filename[image['id']] = image['file_name']

            if "ign" in self.dataset_name:
                json_file = PathManager.get_local_path(self._metadata.json_file)
                with open(json_file, 'r') as f:
                    dataset = json.load(f)
                # Create a dictionary mapping from the dataset
                for image in dataset['images']:
                    ign_id2filename[image['id']] = image['file_name']
            ...
            with open('temp_all_det_cors.txt', 'w') as f2:
                for ix in range(len(data)):
                    if data[ix]['score'] > 0.1:
                        if "rumsey" in self.dataset_name:
                            outstr = '{}: '.format(rumsey_id2filename[data[ix]['image_id']][:-4])
                        elif "ign" in self.dataset_name:
                            outstr = '{}: '.format(ign_id2filename[data[ix]['image_id']][:-4])
                        else:
        ...
        for line in fres:
            ...
            if 'rumsey' in self.dataset_name:
                ...
                filename = filename.split('/')[-1]  # 移除路径
    ...
    def ctc_decode(self, rec):
        ...
        for c in rec:
            ...
            if c < self.voc_size - 1:
                if last_char != c:
                    if self.voc_size == 37 or self.voc_size == 96 or self.voc_size == 148:
                        s += self.CTLABELS[c]
                        last_char = c
                    else:
