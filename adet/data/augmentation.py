...
def gen_crop_transform_with_instance(crop_size, image_size, instances, crop_box=True):
    ...
    if not crop_box:
        ...
        while modified:
            modified, x0, y0, crop_size = adjust_crop(x0, y0, crop_size, instances, image_size)
            num_modifications += 1
...
def adjust_crop(x0, y0, crop_size, instances, image_size, eps=1e-3):
    modified = False
    iteration_count = 0
    ...
    while True:
        for bbox in instances:
            # Expand the crop to include the bbox if it is partially outside
            if bbox[0] < x0 - eps and bbox[2] > x0 + eps:
                additional_width = x0 - bbox[0]
                crop_size[1] += additional_width
                x0 -= additional_width
                modified = True

            if bbox[0] < x1 - eps and bbox[2] > x1 + eps:
                additional_width = bbox[2] - x1
                crop_size[1] += additional_width
                x1 += additional_width
                modified = True

            if bbox[1] < y0 - eps and bbox[3] > y0 + eps:
                additional_height = y0 - bbox[1]
                crop_size[0] += additional_height
                y0 -= additional_height
                modified = True

            if bbox[1] < y1 - eps and bbox[3] > y1 + eps:
                additional_height = bbox[3] - y1
                crop_size[0] += additional_height
                y1 += additional_height
                modified = True

        iteration_count += 1
        if not modified or iteration_count >= 100:
            break
        modified = False  # Reset modification flag for the next iteration

    # Ensure the crop size does not exceed the image dimensions
    x0 = max(0, min(x0, image_size[1] - crop_size[1]))
    y0 = max(0, min(y0, image_size[0] - crop_size[0]))
    crop_size[1] = min(crop_size[1], image_size[1] - x0)
    crop_size[0] = min(crop_size[0], image_size[0] - y0)
